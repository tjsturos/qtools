...
if [[ "$IS_BACKUP_ENABLED" == 'true' ]]; then
    ...
    local_files=$(find "$LOCAL_PATH" -type f -exec stat -f "%N %z" {} +)
    remote_files=$(ssh -i "$SSH_KEY_PATH" -o StrictHostKeyChecking=no "$REMOTE_USER@$REMOTE_URL" "find '$REMOTE_PATH' -type f -exec stat -f \"%N %z\" {} +")

    # Function to check if a file exists in remote files
    file_exists_in_remote() {
        local file=$1
        local size=$2

        while IFS= read -r remote_file; do
            remote_file_name=$(basename "$(echo "$remote_file" | awk '{print $1}')")
    ...
    while IFS= read -r local_file; do
        local_file_name=$(basename "$(echo "$local_file" | awk '{print $1}')")
