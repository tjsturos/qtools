...
display_stats() {
    ...
    if [ $count -gt 0 ]; then
        ...
        if $PRINT_QUIL && [ "$(is_app_finished_starting)" == "true" ]; then
            ...
            output+=("Note: Average reward per frame is calculated only for frames with")
            output+=("      rewards ($reward_total_count frames) that have been received")
            output+=("      in a single frame")
            output+=("      while average reward per second is calculated by taking the")
            output+=("      total rewards over the entire time period divided by the")
            output+=("      total time ($total_time seconds) and extrapolated to an")
            output+=("      hour, day, and month")
            output+=("")
