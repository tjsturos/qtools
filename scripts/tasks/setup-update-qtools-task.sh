...
create_and_load_update_task_plist() {
    ...
    if [ -f "$plist_path" ]; then
        log "Existing update-node task found. Removing and recreating..."
...
<dict>
    <key>Label</key>
    <string>com.qtools.update-qtools</string>
    <key>ProgramArguments</key>
    <array>
        <string>/bin/bash</string>
        <string>-c</string>
        <string>qtools self-update</string>
    </array>
    ...
    <string>$QTOOLS_PATH/update_node.log</string>
...
EOF
    ...
    log "Created and loaded update-node task: $plist_path"
}
