...
set_fresh_proof_batch_check_status() {
    local status=$1
    yq -i ".scheduled_tasks.check_if_fresh_proof_batches.enabled = $status" $QTOOLS_CONFIG_FILE
    echo "Fresh proof batch check has been turned $([[ $status == true ]] && echo "on" || echo "off")."
    qtools update-cron
}

# Check current statistics status
current_status=$(yq '.scheduled_tasks.check_if_fresh_proof_batches.enabled' $QTOOLS_CONFIG_FILE)

# Parse command line arguments
if [[ $# -eq 1 ]]; then
    case $1 in
        --on)
            set_fresh_proof_batch_check_status true
            exit 0
            ;;
        --off)
            set_fresh_proof_batch_check_status false
            exit 0
            ;;
        *)
