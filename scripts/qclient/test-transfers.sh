...
get_total_balance() {
    local total_balance=$(qtools get-tokens ${SKIP_SIG_CHECK:+--skip-sig-check} | awk '{sum += $1} END {print sum}')
    echo $total_balance
}
...
update_suitable_tokens() {
    ...
    suitable_tokens=$(qtools get-tokens ${SKIP_SIG_CHECK:+--skip-sig-check} | grep "0x" | awk -v amount="$AMOUNT" '$1 == amount {print $0}')
