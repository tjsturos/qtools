#!/bin/bash
# HELP: Set the REST listen multiaddr in the node's config file.
# PARAM: --port <port-int> - The port number (optional, defaults to 8338)
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            ...
            echo "Usage: qtools set-rest-multiaddr [--port <port>] [--proto <udp|tcp>] [--ip <ip>]"
            exit 1
            ;;
    esac
done
...
# Build multiaddr
if [ "$PROTO" = "udp" ]; then
    REST_MULTIADDR="/ip4/${IP}/udp/${PORT}/quic-v1"
else
    REST_MULTIADDR="/ip4/${IP}/tcp/${PORT}"
fi
...
if [ "$file_owner" == "quilibrium" ] && [ "$(whoami)" != "root" ]; then
    sudo yq -i '.listenRESTMultiaddr = "'"$REST_MULTIADDR"'"' "$QUIL_CONFIG_FILE"
else
    yq -i '.listenRESTMultiaddr = "'"$REST_MULTIADDR"'"' "$QUIL_CONFIG_FILE"
fi

echo "REST listen multiaddr updated to: $REST_MULTIADDR"
