#!/bin/bash
# HELP: Set the P2P listen multiaddr in the node's config file.
# PARAM: --port <port-int> - The port number (optional, defaults to 8336)
# PARAM: --proto <udp|tcp> - The protocol (optional, defaults to tcp)
# PARAM: --ip <ip-address> - The IP address (optional, defaults to 0.0.0.0)
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            ...
            echo "Usage: qtools set-p2p-listen-multiaddr [--port <port>] [--proto <udp|tcp>] [--ip <ip>]"
            exit 1
            ;;
    esac
done
...
# Build multiaddr
if [ "$PROTO" = "udp" ]; then
    P2P_MULTIADDR="/ip4/${IP}/udp/${PORT}/quic-v1"
else
    P2P_MULTIADDR="/ip4/${IP}/tcp/${PORT}"
fi
...
if [ "$file_owner" == "quilibrium" ] && [ "$(whoami)" != "root" ]; then
    sudo yq -i '.p2p.listenMultiaddr = "'"$P2P_MULTIADDR"'"' "$QUIL_CONFIG_FILE"
else
    yq -i '.p2p.listenMultiaddr = "'"$P2P_MULTIADDR"'"' "$QUIL_CONFIG_FILE"
fi

echo "P2P listen multiaddr updated to: $P2P_MULTIADDR"
