...
modify_config_file() {
    ...
    qtools set-p2p-listen-multiaddr --port 8336 --proto "$LISTEN_ADDR_MODE"

    # Set gRPC listen multiaddr (port defaults to 8337)
    qtools set-grpc-multiaddr

    # Set REST listen multiaddr (port defaults to 8338)
    qtools set-rest-multiaddr
    ...
    if [ "$STATS_ENABLED" = "true" ]; then
        qtools set-stats-multiaddr --enable --url "/dns/stats.quilibrium.com/tcp/443"
    else
        qtools set-stats-multiaddr --disable
    fi
